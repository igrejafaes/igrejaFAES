...
export class CarouselFormModalComponent implements OnInit {
  ...
  onSave() {
    ...
    if(this.selectedFiles){
      this.alert.showAlertWarning('A image deve ser enviada antes de salvar', 'Imagem');
      return;
    }
  ...
  uploadImage() {
    ...
    if(this.carousel.imageName){
      this.upSvc.deleteUpload(this.carousel.imageName, 'carousel')
      .catch((err) => {
        this.alert.showAlertDanger('Não foi possível realizar a troca de imagem...', 'Imagem')
        console.log(err);
        return;
      })
    };
    ...
    this.upSvc.uploadFile(this.currentUpload).then((url)=>{
      ...
      this.alert.showAlertDanger('Não foi possível enviar a imagem...', 'Imagem');
      console.log(err);
    })
  }
